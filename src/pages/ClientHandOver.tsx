...
const initialDeliveryOrders: DeliveryOrder[] = [
	{
		...
		deliveryStatus: 'Готов к выдаче',
	},
	{
		id: 'del_2',
		trackingCode: 'CN-10293-A',
		clientName: 'Мадина Солихова',
		phone: '+992 90 444 5566',
		total: 42.7,
...
export default function ClientDelivery() {
	...
	const handleDeliver = (id: string) => {
		setOrders(prev =>
			prev.map(order =>
				order.id === id ? { ...order, deliveryStatus: 'Выдан клиенту' } : order,
			),
		)
	}
		...
		<div className='w-full p-6 space-y-4'>
			<div className='flex flex-col space-y-1'>
				...
				<p className='text-sm text-muted-foreground'>
					Поиск клиентов, проведение кассовых операций (Payment) и регистрация
					выдачи (Delivery).
				</p>
			</div>
			...
			<Card className='border-zinc-200 dark:border-zinc-800'>
				<CardHeader>
					...
					<CardDescription>
						Управление статусами оплаты и фактическим отпуском посылок со
						склада.
					</CardDescription>
				</CardHeader>
				<CardContent>
									...
									<TableRow key={order.id}>
										...
										<TableCell>
											{order.paymentStatus === 'Оплачено' ? (
												...
												<Badge variant='destructive' className='gap-1'>
													<AlertCircle className='h-3 w-3' /> Долг
												</Badge>
											)}
										</TableCell>
										<TableCell>
										...
										<TableCell className='text-right flex justify-end gap-2'>
											...
											{order.deliveryStatus === 'Готов к выдаче' ? (
												...
												<span className='text-xs text-muted-foreground self-center px-3'>
													Закрыт
												</span>
											)}
										</TableCell>
